...
export default function RefundRequestTable({ title, data }) {

    let searchFields = ['id', 'withdrawId', 'userId', 'orderId', 'orderAmount', 'reason', 'createdDate']
            ...
            <div className="bg-white mt-9 rounded shadow p-4 space-y-8">
                <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex items-center justify-end space-x-4">
                        ...
                        <button onClick={() => sortData()} className="bg-blue-500 rounded-md px-3 py-1 text-black text-sm font-medium">Search</button>
                        <div className="pl-10"> 
                            <BiExport className="hover:text-blue-500 cursor-pointer" onClick={() => downloadExcel("Refund_Request", sortedData)} size={25} />
                        </div>
                    </div>
                </div>
                <div>
                            ...
                            <TableBody>
                                {sortedData.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((item) => (
                                    <TableRow key={item.id}>
                                        ...
                                        <TableCell>{item.withdrawId}</TableCell>
