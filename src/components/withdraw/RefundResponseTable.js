...
export default function RefundResponseTable({ title, data }) {

    let searchFields = ['id', 'refundRequestId', 'userId', 'orderNum', 'orderAmount', 'createdDate']
            ...
            <div className="bg-white mt-9 rounded shadow p-4 space-y-8">
                <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex items-center justify-end space-x-4">
                        ...
                        <div className="pl-10"> 
                            <BiExport className="hover:text-blue-500 cursor-pointer" onClick={() => downloadExcel("Refund-Response", sortedData)} size={25} />
                        </div>
                    </div>
                </div>
                <div>
                            ...
                            <TableBody>
                                {sortedData && sortedData.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((item) => (
                                    <TableRow key={item.id}>
                                        ...
                                        <TableCell>{item.refundRequestId}</TableCell>
                                        <TableCell>{item.userId}</TableCell>
                                        <TableCell>{item.orderNum}</TableCell>
                                        <TableCell>₮{new BigNumber(item.orderAmount).toFormat(0)}</TableCell>
