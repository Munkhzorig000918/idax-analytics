...
export default function WithdrawRequestTable({ title, data }) {

    let searchFields = ['id', 'userId', 'statusId', 'amount', 'bankAccNo', 'orderNum', 'bankAccName', 'errorMessage', 'errorCode', 'createdDate']
            ...
            <div className="bg-white mt-9 rounded shadow p-4 space-y-8">
                <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex items-center justify-end space-x-4">
                        ...
                        <div onClick={() => sortData()} className="bg-blue-500 text-black text-sm font-medium cursor-pointer rounded p-2">Search</div>
                        <div className="pl-10"> 
                            <BiExport className="hover:text-blue-500 cursor-pointer" onClick={() => downloadExcel("Withdraw-Request", sortedData)} size={25} />
                        </div>
                    </div>
                </div>
                <div>
                            ...
                            <TableBody>
                                {sortedData && sortedData.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((item) => (
                                    <TableRow key={item.id}>
                                        ...
                                        <TableCell>{item.bankAccNo}</TableCell>
                                        <TableCell>{item.orderNum}</TableCell>
                                        <TableCell>{item.bankAccName}</TableCell>
