...
export default function SmsRequestTable({ title, data }) {

    let searchFields = ['id', 'username', 'countryCode', 'phoneNumber', 'lang', 'isLocal', 'createdDate']
            ...
            <div className="bg-white mt-9 rounded shadow p-4 space-y-8">
                <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex items-center justify-end space-x-4">
                        ...
                        <button onClick={() => sortData()} className="bg-blue-500 text-black text-sm font-medium rounded-md px-3 py-1">Search</button>
                        <div className="pl-10"> 
                            <BiExport className="hover:text-blue-500 cursor-pointer" onClick={() => downloadExcel("Sms-Request", sortedData)} size={25} />
                        </div>
                    </div>
                </div>
                <div>
                            ...
                            <TableBody>
                                {sortedData.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((item) => (
                                    <TableRow key={item.id}>
                                        ...
                                        <TableCell>{item.isLocal ? '1' : '0'}</TableCell>
                                        <TableCell>{moment(item.createdDate).format("YYYY-MM-DD HH:mm:ss")}</TableCell>
                                    </TableRow>
                                ))}
                            </TableBody>
                        </Table>
                        {sortedData.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).length == 0 && <p className="flex justify-center py-4 text-sm font-medium">Хайлт олдсонгүй</p>}
                    </TableContainer>
